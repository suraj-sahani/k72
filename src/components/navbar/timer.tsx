...
function getMontrealTime(): string {
  const now = new Date()
  ...
  return `MONTREAL_${hh}:${mm}:${ss}`
}

export default function Timer() {
  const [time, setTime] = useState<string>(getMontrealTime())

  useEffect(() => {
    // Align updates exactly to the second boundary
    const tick = () => setTime(getMontrealTime())
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <span className="font-lausanne-regular text-2xl leading-[0.8] uppercase">
        {time}
      </span>
    </div>
  )
